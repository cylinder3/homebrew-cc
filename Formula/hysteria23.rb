class Hysteria23 < Formula
  ...
  def caveats
    <<~EOS
    Create hysteria configure file on `/usr/local/etc/hysteria2/hysteria23.yaml`
    ...
    To start the service, run:
      brew services start hysteria23
    EOS
  end
  ...
  service do
    run [opt_bin/"hysteria-darwin-amd64-avx", "-c", "#{etc}/hysteria2/hysteria23.yaml"]
    run_type :immediate
    keep_alive true
  end
end
